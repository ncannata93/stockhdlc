...
import { Printer, Calendar, Settings, CalendarClock, LogOut } from "lucide-react"
...
export function DailyReport({ report }: Props) {
  ...
  const stats = {
    ...
    firstFloorPax: Array.from(apartmentMap.values())
      .filter((cleaning) => cleaning !== null && cleaning!.apartment.startsWith("1"))
      .reduce((sum, cleaning) => sum + (cleaning!.pax || 0), 0),
    secondFloorPax: Array.from(apartmentMap.values())
      .filter((cleaning) => cleaning !== null && cleaning!.apartment.startsWith("2"))
      .reduce((sum, cleaning) => sum + (cleaning!.pax || 0), 0),
  }
  ...
  const PrintHeader = () => (
    <div className="hidden print:block mb-4 print-header">
      ...
      <div className="grid grid-cols-5 gap-2 mt-2 print-stats">
        ...
        <div className="p-1 text-center border border-black">
          <div className="text-[9px] font-bold">PAX por Piso</div>
          <div className="text-[7px]">1er: {stats.firstFloorPax}</div>
          <div className="text-[7px]">2do: {stats.secondFloorPax}</div>
        </div>
      </div>
    </div>
  )
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between print:hidden">
        ...
        <div className="flex gap-2">
          <Button onClick={handleLogout} variant="outline" size="lg" className="gap-2 bg-transparent text-red-600 hover:bg-red-50 hover:text-red-700">
      ...
      <div id="print-content">
        <div className="grid grid-cols-5 gap-4 mb-4 print:hidden">
          ...
          <Card className="p-3 text-center">
            <div className="space-y-0.5 mb-1">
              <div className="text-lg font-bold text-purple-600">1er piso: {stats.firstFloorPax}</div>
              <div className="text-lg font-bold text-indigo-600">2do piso: {stats.secondFloorPax}</div>
            </div>
            <div className="text-xs text-muted-foreground">PAX por Piso</div>
          </Card>
        </div>
